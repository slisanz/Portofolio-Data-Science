...
`data/raw/e-3.csv` — ~57k buildings with WGS84 coordinates, building category, deployment state, PM reference, operator code. Source: [data.europa.eu — Arcep observatory, dataset 66cc7372](https://data.europa.eu/data/datasets/66cc737273555682b1e9b051?locale=en). Variable dictionary in `dictionnaire-des-variables.pdf` next to it.
...
Notebook 01 (data audit) and 09 (causal sketch) are exploratory — optional.
...
Five pages: **Overview** (headline numbers + freshness), **Map Explorer** (point sample or hex aggregation, commune filter), **Equity Index** (commune ranking + sub-indicator breakdown), **Deployment Predictor** (calibrated `P(lagging)` what-if), **Methodology** (how each metric is built and what it does *not* say).

The dashboard uses a custom dark navy + cyan theme defined in `.streamlit/config.toml`.
