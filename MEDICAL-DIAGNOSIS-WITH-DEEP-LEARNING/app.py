...
def main() -> None:
    st.info(
        "**Note on model size and accuracy.** This demo deliberately uses a "
        "compact model (DenseNet121, around 28 MB, 8 million parameters) so the "
        "app fits within the 1 GB RAM limit of Streamlit Community Cloud's free "
        "tier. A larger ensemble (DenseNet121 + EfficientNetV2-S + ConvNeXt-Tiny, "
        "roughly 225 MB total) would lift test AUC from 0.909 to about 0.95, but "
        "would exceed the free tier's memory budget and crash the app on load. "
        "Because of this trade-off, predictions on this demo may occasionally be "
        "wrong on borderline cases that a larger ensemble would catch. The model "
        "still delivers AUC 0.909, recall 95.1% on PNEUMONIA, and Grad-CAM "
        "overlays for every prediction, which is enough for a screening-style "
        "demonstration. For clinical use, a larger ensemble on dedicated hardware "
